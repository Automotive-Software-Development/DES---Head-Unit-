...
QString WeatherData::maxTemperature() const
{
    return m_maxTemperature;
}

QString WeatherData::feelsLikeTemperature() const
{
    return m_feelsLikeTemperature;
}
...
void WeatherData::setMaxTemperature(const QString &value)
{
    m_maxTemperature = value;
    emit dataChanged();
}

void WeatherData::setFeelsLikeTemperature(const QString &value)
{
    m_feelsLikeTemperature = value;
    emit dataChanged();
}

void WeatherData::setHumidity(const double &value)
{
    m_humidity = value;
    emit dataChanged();
}
...
public:
        ...
        minMsBeforeNewRequest(baseMsBeforeNewRequest)
    {
        ...
        delayedCityRequestTimer.setInterval(1000); // 1 sec
        requestNewWeatherTimer.setSingleShot(false);
        requestNewWeatherTimer.setInterval(1000 * 60); // 1 min
        throttle.invalidate();
...
{
    qDebug() << "got weather network data" << networkReply;
    if (!networkReply)
        return;
    ...
    if (!networkReply->error()) {
        ...
        if (document.isObject()) {
            ...
            QJsonValue val, minTemp, maxTemp, feelLikeTemp, humidity, windSpeed;
            ...
            if (obj.contains(QStringLiteral("main"))) {
                ...
                minTemp = tempObject.value(QStringLiteral("temp_min"));
                maxTemp = tempObject.value(QStringLiteral("temp_max"));
                feelLikeTemp = tempObject.value(QStringLiteral("feels_like"));
                humidity = tempObject.value(QStringLiteral("humidity"));
                d->now.setTemperature(niceTemperatureString(val.toDouble()));
                d->now.setMinTemperature(niceTemperatureString(minTemp.toDouble()));
                d->now.setMaxTemperature(niceTemperatureString(maxTemp.toDouble()));
                d->now.setFeelsLikeTemperature(niceTemperatureString(feelLikeTemp.toDouble()));
                d->now.setHumidity(humidity.toDouble());
            }

            if(obj.contains(QStringLiteral("wind"))) {
                windSpeed = obj.value(QStringLiteral("wind"));
                tempObject = windSpeed.toObject();
                windSpeed = tempObject.value(QStringLiteral("speed"));
                qDebug() << "============Demo==============" << windSpeed;
                d->now.setWindSpeed(windSpeed.toDouble());
            }
        }
    }
    networkReply->deleteLater();
...
{
    qCDebug(requestsLog) << "got forecast" << networkReply;
    if (!networkReply)
        return;
